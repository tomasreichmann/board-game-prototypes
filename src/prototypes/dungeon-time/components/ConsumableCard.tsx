...
export type ConsumableCardProps = React.PropsWithChildren<
    { className?: string } & ConsumableType
>;
...
export default function ConsumableCard({
    ...
    return (
        <Paper
            size="US game"
            className={clsx(
                "ConsumableCard bg-white rounded-sm px-2 pt-2 flex flex-col gap-2",
                className
            )}
        >
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-1">
                <Icon
                    icon={icon}
                    className="w-full max-h-24 text-lightning-1"
                />
            ...
            <div className="flex-1 text-center leading-4 text-md italic">
                {effect && (
                    <RichText iconProps={{ className: "h-8 inline-block" }}>
                        {effect}
                    </RichText>
                )}
            </div>
