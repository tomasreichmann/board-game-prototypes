...
export default function ActionCard({
    ...
    return (
        <Paper
            size="US game"
            className={clsx(
                "ActionCard bg-white rounded-sm p-5 flex flex-col gap-2",
                className
            )}
        >
            <div>
                ...
                {deck && (
                    <div className="text-acid-1 font-dtHeading text-center text-sm">
                        {deck.name}
                    </div>
                )}
            </div>
            ...
            {upgradeOptions.length > 0 && (
                <div className="pt-3">
                    <div className="flex flex-1 w-full flex-row gap-2 mb-1">
                        ...
                        <Icon
                            className="h-4 text-lightning-3"
                            icon="cardExchange"
                        />
                        <div className="flex-1 border-t-2 border-t-slate-400 mt-2"></div>
                    </div>
