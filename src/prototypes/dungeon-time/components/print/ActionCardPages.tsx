...
export default function ActionCardPages() {
    ...
    const allActionDeckUpgrades = decks.length > 0 ? getDeckUpgradesDeep(decks.flatMap((deck) => deck.actions)) : [];

    const cardTypeCounts = countByProperty(allActionDeckUpgrades, "sourceSlug");
    const cardsOfTypeLimits = mapValues(cardTypeCounts, (cardCount) => Math.min(cardCount, MAX_IDENTICAL_UPGRADES));
    const cardsOfTypeLeft = { ...cardsOfTypeLimits };

    const unsortedActionDeckUpgrades = allActionDeckUpgrades.reduce((actionDeckUpgrades, action) => {
        const { sourceSlug } = action;
        if (cardsOfTypeLeft[sourceSlug] > 0) {
            const slug = sourceSlug + "-" + (cardsOfTypeLimits[sourceSlug] - cardsOfTypeLeft[sourceSlug]);
    ...
    const actionDeckUpgrades = [...unsortedActionDeckUpgrades].sort((a, b) => {
        return (
            unsortedActionDeckUpgrades.findIndex((action) => action.sourceSlug === a.sourceSlug) -
            unsortedActionDeckUpgrades.findIndex((action) => action.sourceSlug === b.sourceSlug)
        );
    });
    ...
    return (
        <>
            <div className="flex flex-row justify-start w-full print:hidden gap-2">
                <DataToggle data={cardTypeCounts} buttonContent="Total Upgrade Cards by type" initialCollapsed />
                <DataToggle data={cardsOfTypeLimits} buttonContent="Filtered Upgrade Cards by type" initialCollapsed />
            </div>
            {decks.map(({ actions, name }) => (
                <>
                    {chunk(actions, CARDS_PER_PAGE).map((actions, actionsPageIndex) => (
                        <PrintPage key={"action-deck-page-" + actionsPageIndex}>
                            <div className="flex flex-wrap content-center items-center">
                                {actions.map((action) => (
                                    <ActionCard key={action.slug} {...action} className="relative">
                                        <PrintMarkerCorners />
                                    </ActionCard>
                                ))}
                                <h2 className="text-2xl font-dtHeading text-blood-3 w-full text-center">
                                    {name} {actionsPageIndex + 1}/{Math.ceil(actions.length / CARDS_PER_PAGE)}
                                </h2>
                            </div>
                        </PrintPage>
                    ))}
            ...
            />
            {chunk(actionDeckUpgrades, CARDS_PER_PAGE).map((actions, actionsPageIndex) => (
                <PrintPage key={"action-deck-upgrades-page-" + actionsPageIndex}>
                    <div className="flex flex-wrap content-center items-center">
                        {actions.map((action) => (
                            <ActionCard key={action.slug} {...action} className="relative">
                                <PrintMarkerCorners />
                        ...
                        <h2 className="text-2xl font-dtHeading text-blood-3 w-full text-center">
                            ...
                            {Math.ceil(actionDeckUpgrades.length / CARDS_PER_PAGE)}
                        </h2>
                    </div>
                </PrintPage>
            ))}
