...
export default function EnemyPages() {
    ...
    const consumables = data ? multiplyByCount(data, "cardCount", defaultCountAdapter) : [];
    const totalPages = Math.ceil(consumables.length / CARDS_PER_PAGE);
    return (
        <>
            {chunk(consumables, CARDS_PER_PAGE).map((consumables, pageIndex) => (
                <PrintPage key={"enemy-cards-page-" + pageIndex}>
                    <div className="flex flex-wrap content-center items-center">
                        {consumables.map((enemy) => (
                            <ConsumableCard key={enemy.slug} {...enemy} className="relative">
                                <PrintMarkerCorners />
                            </ConsumableCard>
                        ))}
