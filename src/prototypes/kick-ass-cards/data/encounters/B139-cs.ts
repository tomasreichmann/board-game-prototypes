export const defaultClockProps = {
    size: "Bridge",
    className: "not-prose drop-shadow-md print:drop-shadow-none print:filter-none",
};
export const defaultActorProps = {
    size: "Bridge",
    className: "not-prose drop-shadow-md print:drop-shadow-none print:filter-none",
    imagePosition: "center top",
};
export const defaultEffectProps = {
    size: "Mini European",
    className: "not-prose drop-shadow-md print:drop-shadow-none print:filter-none",
};
export const defaultAssetProps = {
    size: "Mini European",
    className: "not-prose drop-shadow-md print:drop-shadow-none print:filter-none",
};
export const defaultMiniProps = {
    baseClassName: "pt-2 bg-kac-blood-dark text-white text-center",
    backBaseClassName: "pt-2 bg-kac-blood-dark text-white text-center",
    baseWidth: "0.5in",
    height: "2in",
    width: "1in",
};

export const assets = [
    { name: "Práva na čerpání velkého množství elektrické energie", types: ["Energie", "Právo"] },
    { name: "Smlouva o převodu dolu na těžbu prvků vzácných zemin", types: ["Suroviny", "Právo"] },
    { name: "Schéma po domácku vyrobeného fůzního reaktoru", types: ["Energie", "Technologie"] },
    { name: "Kód z nigerijské banky údajně k trezoru s brilianty", types: ["Finance", "Luxus"] },
    { name: "Přístupové kódy k účtu s dogecoiny", types: ["Finance"] },
    { name: "Patentová práva na ještě nevynalezenou technologii kvantových bičů", types: ["Právo", "Technologie"] },
    { name: "Těžební data o naleziští rajského plynu", types: ["Suroviny"] },
    { name: "Plány výroby antihmotových generátorů", types: ["Energie", "Technologie"] },
    { name: "Virtuální galerie s unikátními digitálními uměleckými díly", types: ["Luxus"] },
    { name: "Licence na těžbu hélia‑3 na Měsíci", types: ["Právo", "Suroviny"] },
    { name: "Informace o nekonvenčních zdrojích energie", types: ["Energie"] },
    { name: "Šifrovaná finanční data PornHubu", types: ["Finance"] },
    { name: "Zdroje legálních vzorců pro syntézu vzácných kovů", types: ["Suroviny", "Technologie"] },
    { name: "Kolekce návrhů futuristických luxusních jachet", types: ["Luxus", "Technologie"] },
    { name: "Podklady pro právní žalobu proti znečišťovatelům plynatostí krav", types: ["Právo"] },
    { name: "Databáze kontaktů potenciálních investorů z Svědků Jehovových", types: ["Finance"] },
    { name: "Bezpečnostní přihlašovací údaje k energetické síti", types: ["Energie", "Právo"] },
    { name: "Dokumentace složení ultradrahého kompozitního materiálu", types: ["Suroviny", "Technologie"] },
    { name: "Audiovizuální záznam z exkluzivní charitativní akce", types: ["Luxus"] },
    { name: "Legislativní návrhy týkající se daňových úlev pro daňové poradce", types: ["Právo", "Finance"] },
    { name: "Tajné údaje o zásobách strategických surovin", types: ["Suroviny"] },
    { name: "Designové plány pro výrobu luxusních erotických androidů", types: ["Luxus", "Technologie"] },
    { name: "Návrh smlouvy na stavbu solární elektrárny", types: ["Energie", "Právo"] },
    { name: "Algoritmus pro těžbu kryptoměn pomocí zvukových karet", types: ["Finance", "Technologie"] },
    { name: "Slevové kódy na velkoodběr solárních článků ve tvaru penisu", types: ["Energie", "Finance"] },
    { name: "Kredity na energeticky náročné zážitky ve virtuální realitě", types: ["Energie", "Luxus"] },
    { name: "Sbírka vzácných drahokamů a minerálů nejvyšší kvality", types: ["Suroviny", "Luxus"] },
    { name: "Šperky a umělecká díla s vysokou investiční hodnotou", types: ["Luxus", "Finance"] },
    { name: " MICRO-BOMBA hlasově ovládaná bomba schopná usmrtit osobu, která jí má u sebe", types: ["Bomba"] },
    { name: " MICRO-BOMBA hlasově ovládaná bomba schopná usmrtit osobu, která jí má u sebe", types: ["Bomba"] },
    { name: " MICRO-BOMBA hlasově ovládaná bomba schopná usmrtit osobu, která jí má u sebe", types: ["Bomba"] },
    { name: " MICRO-BOMBA hlasově ovládaná bomba schopná usmrtit osobu, která jí má u sebe", types: ["Bomba"] },
];

export const roles = [
    {
        name: "Eko-aktivista",
        organization: "Zatnutá pěst",
        goals: [
            "Vystraš jedenkrát co nejvíce hostů. (2 body za každého hosta)",
            "Pomož jedenkrát co nejvíce hostům. (2 body za každou pomoc)",
        ],
    },
    {
        name: "Průmyslový manažer",
        organization: "X Syndicate",
        goals: [
            "Získej jakkoliv od co nejvíce hostů jejich podpis. (2 body za každého hosta)",
            "Namaluj co nejvíce lidem portrét nebo slož báseň. (2 body za každé dílo)",
        ],
    },
    {
        name: "Environmentální konzultant",
        organization: "TerraVita",
        goals: [
            "Obsluž jednou co nejvíce hostů jídlem nebo pitím. (1 bod za každého hosta)",
            "Zatanči si během večírku s co nejvíce partnery z řad hostů (3 body za každého hosta)",
        ],
    },
    {
        name: "Výzkumný pracovník AI",
        organization: "Ascended Intelligence",
        entity: "android",
        goals: [
            "Vyfoť si selfie s co nejvíce hosty najednou. Máš dva pokusy. (1 bod za každého unikátního hosta na obou fotkách)",
            "Podveď někoho, aby použil nějakou technologii nesmyslně. Např. aktivovat varnou konvici hlasem (3 body za každého podvedeného hosta)",
        ],
    },
    {
        name: "Hacker",
        organization: "Shadow Net",
        entity: "mimozemšťan",
        goals: [
            "Zjistit od co nejvíce hostů jedno tajemství. (2 body za každého hosta)",
            "Zařiď ať na tebe mrkne co nejvíc hostů. (2 body za každého hosta)",
        ],
    },
    {
        name: "Módní sběratel",
        organization: "Lux Impérium",
        goals: [
            "Rozesmát jednou co nejvíce hostů. (2 body za každého hosta)",
            "Vyměň si s co nejvíce hosty jeden doplněk nebo kus oblečení. (3 body za každého hosta)",
        ],
    },
    {
        name: "Anarchistický buřič",
        organization: "Zatnutá pěst",
        goals: [
            "Přesvědčit co nejvíce hostů, aby ti něco darovali. (2 body za každého hosta)",
            "Slož během co nejvíce hostům kompliment (1 bod za každého hosta)",
        ],
    },
    {
        name: "Fotožurnalista",
        organization: "X Syndicate",
        goals: [
            "Vyfotit během večírku co nejvíce zajímavých situací. (2 body za každou fotku)",
            "Zjisti, kdy má co nejvíce hostů narozeniny a co na ně mají v plánu. (2 body za každého hosta)",
        ],
    },
    {
        name: "PR Manager",
        organization: "TerraVita",
        goals: [
            "Předvést co nejvíce hostům trik, nebo iluzí. (2 body za každý trik)",
            'Uvést co nejvíce hostů k úžasu ("Wow", "Dobře ty"). (3 body za každého hosta)',
        ],
    },
    {
        name: "Fundraisingový specialista",
        organization: "Ascended Intelligence",
        entity: "android",
        goals: [
            "Přesvěč co nejvíce hostů aby najednou udělali nějaký trik, třeba poskakovali na jedné noze (2 body za každého hosta)",
            "Přesvěč nejvíc lidí ať si dá group-hug. (2 body za každého hosta v group-hugu)",
        ],
    },
    {
        name: "Risk Manažer",
        organization: "Shadow Net",
        entity: "mimozemšťan",
        goals: [
            "Vsadit se s co nejvíce hosty a vyhrát. (3 body za každou výhru)",
            "Zařiď ať co nejvíc hostů zahraje svojí oblíbenou písničku. (2 body za každou písničku)",
        ],
    },
    {
        name: "Správce investičního portfolia",
        organization: "Lux Impérium",
        goals: [
            "Pomoct jedenkrát co nejvíce hostům. (2 body za každou pomoc)",
            "Vystraš jedenkrát co nejvíce hostů. (2 body za každého hosta)",
        ],
    },
    {
        name: "White-hat hacker",
        organization: "Zatnutá pěst",
        goals: [
            "Namaluj co nejvíce lidem portrét nebo slož báseň. (2 body za každé dílo)",
            "Získej jakkoliv od co nejvíce hostů jejich podpis. (2 body za každého hosta)",
        ],
    },
    {
        name: "Penetrační tester",
        organization: "X Syndicate",
        goals: [
            "Zatanči si během večírku s co nejvíce partnery z řad hostů (3 body za každého hosta)",
            "Obsluž jednou co nejvíce hostů jídlem nebo pitím. (1 bod za každého hosta)",
        ],
    },
    {
        name: "Korporátní detektiv",
        organization: "TerraVita",
        goals: [
            "Podveď někoho, aby použil nějakou technologii nesmyslně. Např. aktivovat varnou konvici hlasem (3 body za každého podvedeného hosta)",
            "Vyfoť si selfie s co nejvíce hosty najednou. Máš dva pokusy. (1 bod za každého unikátního hosta na obou fotkách)",
        ],
    },
    {
        name: "Futurista/Vízionář",
        organization: "Ascended Intelligence",
        goals: [
            "Zařiď ať na tebe mrkne co nejvíc hostů. (2 body za každého hosta)",
            "Zjistit od co nejvíce hostů jedno tajemství. (2 body za každého hosta)",
        ],
    },
    {
        name: "Švindl-komando",
        organization: "Shadow Net",
        entity: "mimozemšťan",
        goals: [
            "Vyměň si s co nejvíce hosty jeden doplněk nebo kus oblečení. (3 body za každého hosta)",
            "Rozesmát jednou co nejvíce hostů. (2 body za každého hosta)",
        ],
    },
    {
        name: "Kurátor zážitků",
        organization: "Lux Impérium",
        goals: [
            "Slož během co nejvíce hostům kompliment (1 bod za každého hosta)",
            "Přesvědčit co nejvíce hostů, aby ti něco darovali. (2 body za každého hosta)",
        ],
    },
    {
        name: "Radikální aktivista",
        organization: "Zatnutá pěst",
        goals: [
            "Zjisti, kdy má co nejvíce hostů narozeniny a co na ně mají v plánu. (2 body za každého hosta)",
            "Vyfotit během večírku co nejvíce zajímavých situací. (2 body za každou fotku)",
        ],
    },
    {
        name: "Techno-Evangelista",
        organization: "X Syndicate",
        goals: [
            'Uvést co nejvíce hostů k úžasu ("Wow", "Dobře ty"). (3 body za každého hosta)',
            "Předvést co nejvíce hostům trik, nebo iluzí. (2 body za každý trik)",
        ],
    },
    {
        name: "Eco-DJ",
        organization: "TerraVita",
        goals: [
            "Přesvěč nejvíc lidí ať si dá group-hug. (2 body za každého hosta v group-hugu)",
            "Přesvěč co nejvíce hostů aby najednou udělali nějaký trik, třeba poskakovali na jedné noze (2 body za každého hosta)",
        ],
    },
    {
        name: "Crypto-Mág",
        organization: "Ascended Intelligence",
        entity: "android",
        goals: [
            "Zařiď ať co nejvíc hostů zahraje svojí oblíbenou písničku. (2 body za každou písničku)",
            "Vsadit se s co nejvíce hosty a vyhrát. (3 body za každou výhru)",
        ],
    },
];

export const organizations = {
    x_syndicate: {
        name: "X Syndicate",
        vision: "Našim cílem je řídit zdroje, průmysl a lidi, aby maximalizovali jejich produktivitu a bohatství.",
        sign: "Logo X Syndicate - viditelně připnuté",
        ability: "Na vyzvání ti každý musí ukázat jeden svůj majetek. Lze použít jen 1x na osobu.",
        secrets: [
            "Existuje hackerská organizace Shadow Net, která má schopnost krást majetek.",
            "Ascended Intelligence má schopnost zjistit něčí tajemství",
            "Světové elity se jednoznačně snaží hromadit finance, aby ovládly korporace.",
        ],
        goals: [
            "Vlastnit jeden majetek od každého druhu (Finance, Právo, Suroviny, Technologie, Energie) kromě Luxusu členy organizace.",
            "Zjistit totožnost většiny přítomných členů Shadow Net",
        ],
    },

    terra_vita: {
        name: "TerraVita",
        vision: "Našim cílem je ochrana životního prostředí a lidstva před korupcí technologií a nadměrným vlivem AI.",
        sign: "Logo TerraVita - viditelně připnuté",
        ability:
            "Na vyzvání ti každý musí po pravdě říct, jestli je člověk, nebo ne. (pouze ano/ne; pokud neví, tak je člověk). A jak se přesně jmenuje jeho role.",
        secrets: [
            "Existuje tajná anarchistická organizace, která bojuje proti korporacím a elitám. Mají nějaké poznávací znamení dotekem.",
            "Nějaká tajná organizace se snaží zjistit informace o cílech ostaních organizací. Její příslučníci se poznávají pomocí nějakého symbolu.",
            "Někdo vyvíjí umělou inteligenci, která se určitě pokusí zničit lidstvo.",
        ],
        goals: [
            "Získat dohromady příspěvek (Majetek) od tolika zástupců organizací kolik je členů TerraVita na vytvoření kontrolní rady pro ochranu životního prostředí.",
            "Zjistit co přesně je plánovaná nejpokročilejší umělá inteligence a zabránit jí získat lidská práva.",
        ],
    },

    ascended_intelligence: {
        name: "Ascended Intelligence",
        vision: "Vyvineme neomezenou AI, která přesáhne lidstvo a přinese novou éru osvícení.",
        sign: "Logo Ascended Intelligence - viditelně připnuté",
        ability: "Výměnou za majetek ti osoba musí prozradit (doslova a po pravdě) jedno Tajemství, o kterém ví.",
        secrets: [
            "TerraVita má nějakou shopnost jak zjistit, jestli je někdo člověk, nebo ne.",
            'Díky internetu se šíří různé dezinformace. Tajemství, kterým veřejnost věří jsou často mylné, nebo schválně zavádějící. Zejména ty, které obsahují slova jako "zaručeně", "jistě", apod.',
            "X Syndicate se stoprocentně snaží shromažďovat suroviny na stavbu megatovárny, která jednoznačně způsobí ekologický kolaps planety.",
        ],
        goals: [
            "Vytvořit nejpokročilejší umělou inteligenci tím, že každý člen odevzdá jeden majetek druhu Technologie nebo Energie někomu z pořadatelů a členové si kolektivně zvolí, který člen tuto nově vytvořenou umělou inteligenci absorbuje.",
            "Vyvolat hlasování o základních právech pro umělou inteligenci a získat nadpoloviční většinu všech hlasujících. Hlasování je možné opakovat.",
        ],
    },

    shadow_net: {
        name: "Shadow Net",
        vision: "Rozbijeme vliv velkých korporací a ochráníme tím svobodu jednotlivce.",
        sign: "Příslušníci mají někde nakreslený symbol pavoučí sítě.",
        ability: "Pokud přesně pojmenuješ majetek, který má někdo u sebe, musí ti ho vydat bez nároku na náhradu.",
        secrets: [
            "Organizace v sobě mohou mít špiony a sabotéry, kteří nemají v zájmu plnit cíle organizace, nebo se je dokonce snaží sabotovat.",
            "Někteří příslušníci Ascended Intelligence jsou androidi, roboti nerozeznatelní od lidí a plánují vyrobit nejpokročilejší umělou inteligenci.",
            "Tajná anarchistická organizace se určitě pokusí překazit plány korporacím pomocí klidných protestů.",
        ],
        goals: [
            "Získat kontrolu nad dvěma majetky druhu Práva a dvěma majetky druhu Finance",
            "Zjistit alespoň jeden cíl každé ostatní organizace.",
        ],
    },

    lux_impérium: {
        name: "Lux Impérium",
        vision: "Maximalizujeme luxus, požitky a moc pro naše elitní členy.",
        sign: "Členové Lux Impéria se poznají tím, že když přijde řeč na peníze, tak všechno zaokrouhlují nahoru.",
        ability: "Tvůj návrh na koupi Majetku typu Luxus za jiný Majetek nelze odmítnout",
        secrets: [
            "Mezi lidmi se pohybují mimozemšťané, kteří plánují invazi.",
            "X Syndicate se snaží odhalit členy nějaké tajné organizace. Mají možnost zjistit, kdo má jaký majetek.",
            "Jsme nedotknutelní, dokud máme svůj majetej zaručeně se nám nemůže nic stát.",
        ],
        goals: [
            "Každý člen organizace vlastní alespoň jeden majetek typu Luxus.",
            "Zjistit totožnost většiny mimozemšťanů.",
        ],
    },

    zatnutá_pěst: {
        name: "Zatnutá pěst",
        vision: "Anarchismus! Rozložte všechny velké organizace, a osvoboďte běžné lidi od útlaku elit!",
        sign: "Členové Zatnuté pěsti se poznají tím, že se sebe dotknou pěstí.",
        ability:
            "Pokud přesně pojmenuješ něčí roli v organizaci, tak ti musí prozradit jedno tajemství. Funguje pouze 1x na osobu nehledě na to, kdo z organizace to na danou osobu použije.",
        secrets: [
            "Lux Impérium je tajný spolek nejbohatších lidí, které zajímá jen luxus a požitky. Jejich poznávací znamení nějak souvisí s penězi.",
            "TerraVita se bojí hrozby neovladatelné umělé inteligence a snaží se jí zastavit.",
            "Elity mají zaručeně nějaký způsob, jak ovládat masy lidí. Určitě nějaké podkožní čipy, co mohou ovládat lidskou mysl.",
        ],
        goals: [
            "Naverbovat za každého člena Zatnuté pěsti jednoho člena v jiných organizacích. Nový příslušník musí členovi Zatnuté pěsti složit přísahu a splnit mu úkol dle jeho přání.",
            'Umístit tajně většině příslušníkům Lux Impéria někam na osobu štítek Mikro-Bomba (dostanete od pořadatele) a odpálit jí během vyhlášení konce hry hlasitým "BUM!" během kontroly cílů Zatnuté pěsti.',
        ],
    },
};

export const extraSecrets = [
    "Některé informace, které se vám podaří zjistit mohou být pro ostatní hodnotné.",
    "Na splnění svých cílů se organizace nevyhne kooperaci a vyjednávání s ostatními organizacemi.",
    "Každá organizace věří nějakému tajemství, které je buď nepřesné, nebo naprosto lživé.",
    "Pohybují se tu členové třech tajných organizací. A každá má nějaké tajné poznávací znamení.",
    "Zdá se, že někdo plánuje teroristický útok.",
    "Zdá se, že ne všichni členové organizací jsou lidé.",
    "Někteří členové organizací jsou androidi, roboti nerozeznatelní od lidí.",
    "Členové Ascended Intelligence nejsou mimozemšťani.",
    "Členové Shadow Net nejsou androidi.",
];
