...
This adventure uses standard Mighty Decks rules extended with following rules.
...
<table>
  ...
  <tbody>
  {wealthData.map((row, rowIndex) => (
    <tr>
      <td>{rowIndex}</td>
      <td>{row.description}</td>
      <td>{row.incomeRequirements}</td>
      <td>{row.incomeDescription}</td>
      <td>{row.savingsDuration}</td>
    </tr>)
    )
  }
  </tbody>
