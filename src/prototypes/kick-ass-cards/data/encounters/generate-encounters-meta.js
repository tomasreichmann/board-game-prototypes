...
mdxFiles.forEach((file) => {
    ...
    if (!frontmatter.title || frontmatter.title.trim() === "") {
        console.log(`⏩ Skipping ${path.basename(file)}, title is missing or empty.`);
        return;
    }
...
export const encountersMap: { [key: string]: EncounterDefinition } = ${JSON.stringify(encountersMap, null, 4)};
`;

fs.writeFileSync(outputFile, content, "utf8");

console.log(`✅ Generated encounter metadata for ${Object.keys(encountersMap).length} files in ${outputFile}`);
